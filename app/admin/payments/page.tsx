...
export default function AdminPaymentsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'approved': return <Badge className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 hover:bg-emerald-500/20"><CheckCircle className="w-3 h-3 mr-1" /> Paid</Badge>;
      case 'rejected': return <Badge className="bg-rose-500/10 text-rose-600 border-rose-500/20 hover:bg-rose-500/20"><XCircle className="w-3 h-3 mr-1" /> Rejected</Badge>;
      case 'exempt': return <Badge className="bg-purple-500/10 text-purple-600 border-purple-500/20"><ShieldCheck className="w-3 h-3 mr-1" /> Exempt</Badge>;
      default: return <Badge className="bg-amber-500/10 text-amber-600 border-amber-500/20 hover:bg-amber-500/20"><Clock className="w-3 h-3 mr-1" /> Pending</Badge>;
    }
  };
    ...
    <Card key={p.id} className="mb-4 bg-card border border-border/50 shadow-sm" onClick={() => setSelectedPaymentId(p.id)}>
      <CardContent className="p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground bg-secondary/10 p-2 rounded-lg">
          ...
          <div>
            <span className="block font-medium text-foreground">File</span>
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 p-5 pb-12 animate-fade-in sm:p-8">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-muted-foreground text-lg">Review payment receipts and financial activity.</p>
      </div>
      ...
      <div className="space-y-4 pt-4">
        <div className="flex flex-col sm:flex-row gap-3 bg-card p-4 rounded-xl border border-border/50 shadow-sm">
          <div className="relative flex-1">
            ...
            <Input placeholder="Search by name or email..." className="pl-9 bg-background border-border/50 h-10" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="w-full sm:w-[200px] bg-background border-border/50 h-10">
              <div className="flex items-center gap-2"><Filter className="w-4 h-4" /><SelectValue placeholder="Filter by status" /></div>
            </SelectTrigger>
            <SelectContent>
        ...
        {isLoading ? <TableSkeleton cols={6} mobileCards={true} showTitle={false} /> : (
          <>
            <div className="block md:hidden">
              {payments.length === 0 ? <div className="text-center py-8 text-muted-foreground">No payments found.</div> : payments.map(renderMobileCard)}
            </div>
            ...
            <Card className="hidden md:block border-border/50 bg-card overflow-hidden shadow-sm">
              <Table>
                ...
                <TableBody>
                  {payments.length === 0 ? (
                    <TableRow><TableCell colSpan={6} className="h-48 text-center text-muted-foreground">No payments found.</TableCell></TableRow>
                  ) : (
                    payments.map((p) => (
                      <TableRow key={p.id} className="cursor-pointer hover:bg-muted/30 transition-colors group" onClick={() => setSelectedPaymentId(p.id)}>
                        ...
                        <TableCell className="text-muted-foreground text-sm">{new Date(p.created_at).toLocaleString('en-GB')}</TableCell>
                        ...
                        <TableCell className="text-right"><Button variant="ghost" size="sm" className="h-8">Review <ArrowUpRight className="w-3.5 h-3.5 ml-2" /></Button></TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
