...
export async function POST(request: Request) {
    ...
    try {
        ...
        // --- RESTRICTION: Only Delegates/Press/Observers/Chairs can be assigned ---
        // Previously only 'delegate' slug was checked. Now we verify the actual role.
        // Allow assignment if user is explicitly a 'delegate' OR if they are 'applicant' but applied for delegate form.
        // Also allow chairs/deputies to be assigned (technically they lead, but are members in `committee_members` too).

        const allowedRoles = ['delegate', 'committee_chairman', 'deputy_chair'];
        const isAllowedRole = allowedRoles.includes(targetUser.role);

        // Fallback check for applicants who applied as delegate (legacy or pre-approval assignment)
        ...
        const isDelegateApplicant = targetUser.role === 'applicant' && applicantType === 'delegate';
...
// Change Log:
// - Updated role check logic to prioritize `user.role` (delegate, chair, deputy) instead of just relying on `form.slug`.
// - Maintained fallback support for `applicant` role if `form.slug === 'delegate'` to allow pre-approval assignment if workflow requires it.