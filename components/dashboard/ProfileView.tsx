  ...
  const IDCardContent = ({ expanded = false }) => (
    <div className="relative z-10 h-full flex flex-col p-6">
      <div className="flex justify-between items-start mb-auto">
        <div>
          <motion.div layoutId="card-label" className="text-[10px] font-bold text-primary tracking-[0.2em] uppercase mb-1">Katılımcı Kartı</motion.div>
          <motion.div layoutId="card-event" className="text-xl font-display font-bold tracking-tight">ATAGÇ 2026</motion.div>
        </div>
        <motion.img 
          layoutId="card-logo" 
          src="/logo.webp" 
      ...
      <div className="flex flex-col items-center justify-center space-y-6 my-6">
        <motion.div layoutId="card-qr-bg" className="p-3 bg-white rounded-xl shadow-lg">
          <motion.img
            layoutId="card-qr"
            src={`https://api.qrserver.com/v1/create-qr-code/?size=${expanded ? '300x300' : '150x150'}&data=${user.id}&bgcolor=ffffff`}
            alt="User ID QR"
            className={cn("mix-blend-multiply transition-all", expanded ? "w-64 h-64" : "w-32 h-32")}
          />
        </motion.div>
        <div className="text-center space-y-1">
          <motion.div 
            layoutId="card-name" 
            className={cn("font-bold truncate max-w-[200px] mx-auto", expanded ? "text-2xl" : "text-lg")}
          >
            {user.full_name}
          </motion.div>
          <motion.div layoutId="card-id" className="text-xs text-slate-400 font-mono tracking-widest">
      ...
      <div className="mt-auto pt-4 border-t border-white/10 flex justify-between items-end text-xs text-slate-300 font-medium">
        <div className="flex flex-col">
          ...
          <motion.span layoutId="card-role" className="capitalize text-white">{user.role}</motion.span>
        </div>
        <motion.div layoutId="card-status" className="flex items-center gap-1.5 text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded-full border border-emerald-500/20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="flex flex-col gap-6 relative z-10">
          ...
          <div className="relative w-full aspect-[3/4.8] sm:aspect-[1.586/1] lg:aspect-[3/4.8]">
            ...
            <div className={cn("text-center text-xs text-muted-foreground px-4 mt-4 transition-opacity", isExpanded ? "opacity-0" : "opacity-100")}>
            ...
            <AnimatePresence>
              {isExpanded && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                  ...
                  <motion.div
                    layoutId="card-container"
                    className="w-full max-w-[380px] aspect-[3/4.8] rounded-3xl overflow-hidden relative shadow-2xl ring-1 ring-white/10 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 text-white z-50"
                  >
                    {/* Background Decorations (Replicated) */}
                    <div className="absolute top-0 right-0 w-32 h-32 bg-primary/20 rounded-full blur-[60px] -mr-10 -mt-10" />
                    <div className="absolute bottom-0 left-0 w-24 h-24 bg-blue-500/20 rounded-full blur-[50px] -ml-5 -mb-5" />
...
// Change Log:
// - Added `layoutId` to internal elements of `IDCardContent` to enable true shared element transitions (morphing) instead of cross-fading.
// - Changed `IDCardContent` elements to use `motion.div`, `motion.img`, etc.
// - Updated main render logic: The collapsed card is removed from the DOM (replaced by invisible placeholder) when expanded, transferring layout ownership to the modal.
// - Moved the "Maximize" icon to the center of the card in collapsed state to prevent blocking the logo.
// - Styled the "Close" button in expanded state with a darker backdrop (`bg-black/30`) to ensure visibility over content without clashing.