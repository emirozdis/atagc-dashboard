...
export default function RavenPortalShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#08070D] text-[#F5F3FF] lg:flex">
      {sidebarVisible && <aside className="border-b border-white/10 bg-[#12101A] lg:min-h-screen lg:w-72 lg:border-b-0 lg:border-r">
        <div className="flex items-center gap-3 px-5 py-5 lg:px-6 lg:py-7">
          <Image src="/ravenmun-logo.jpg" width={34} height={34} alt="RavenMUN logo" className="h-[34px] w-[34px] rounded-full object-cover" />
          <div>
            <Link href="/portal" className="raven-template-brand text-lg font-bold tracking-wide">RAVENMUN</Link>
            <p className="mt-0.5 text-xs text-[#9CA3AF]">Participant portal</p>
          </div>
        </div>
        <div className="hidden px-6 pb-7 lg:block">
          <div className="rounded-2xl border border-[#7C3AED]/20 bg-[#7C3AED]/10 p-4">
            <div className="flex items-center gap-3">
              <div className="rounded-xl bg-[#C4B5FD]/15 p-2 text-[#C4B5FD]"><UserRound className="h-4 w-4" /></div>
              <div className="min-w-0">
                <p className="truncate text-sm font-medium">{session?.user?.name || session?.user?.email}</p>
                <p className="text-xs text-[#9CA3AF]">{roleLabels[role] || role}</p>
              </div>
            </div>
          </div>
        </div>
        <nav className="flex gap-1 overflow-x-auto px-3 pb-3 lg:block lg:space-y-1 lg:px-4 lg:pb-0">
          {items.map(({ href, label, icon: Icon }) => {
            ...
            return <Link key={href} href={href} className={`flex shrink-0 items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${active ? "bg-[#7C3AED]/20 text-[#F5F3FF]" : "text-[#9CA3AF] hover:bg-white/5 hover:text-white"}`}><Icon className="h-4 w-4" />{label}</Link>;
          })}
        </nav>
        <div className="hidden px-4 pt-8 lg:block">
          <button onClick={() => signOut({ callbackUrl: "/login" })} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-[#9CA3AF] hover:bg-white/5 hover:text-white"><LogOut className="h-4 w-4" />Sign out</button>
        </div>
      </aside>}
      <div className="min-w-0 flex-1">
        <header className="flex items-center justify-between border-b border-white/10 px-5 py-5 sm:px-8">
          <div className="flex items-center gap-3"><button aria-label={sidebarVisible ? "Hide navigation" : "Show navigation"} onClick={() => setSidebarVisible((visible) => !visible)} className="rounded-xl border border-white/10 p-2 text-[#C4B5FD] hover:bg-white/5">{sidebarVisible ? <PanelLeftClose className="h-4 w-4" /> : <PanelLeftOpen className="h-4 w-4" />}</button><div><p className="raven-template-brand text-xs uppercase tracking-[0.24em]">RavenMUN 2026</p><p className="mt-1 text-sm text-[#C3C7D1]">Your conference workspace</p></div></div>
          <button aria-label="Open profile and devices" onClick={() => router.push("/profile#devices")} className="rounded-xl border border-white/10 p-2 text-[#C4B5FD] hover:bg-white/5"><Settings className="h-4 w-4" /></button>
        </header>
        <main>{children}</main>
      </div>
    </div>
